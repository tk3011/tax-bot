...
def show_receipt_scanner(db, Receipt):
    ...
    st.markdown("*Upload a receipt photo - AI reads the text automatically*")
        ...
        if uploaded_file:
            ...
            with st.spinner("🔍 Reading text from image..."):
                try:
                    ...
                    if amount:
                        st.success(f"✅ Detected: ${amount:.2f} from {vendor or 'Unknown'}")
                    else:
            ...
            with col1:
                ...
                date = st.date_input("Date", datetime.now(), key="dt")
            with col2:
                vendor_input = st.text_input("Vendor", value=vendor, key="ven")
                category = st.selectbox("Category", ["Office supplies", "Travel", "Meals", "Software", "Rent", "Other"], key="cat")
